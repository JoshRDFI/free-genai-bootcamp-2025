...
# Constants
EMBEDDING_DATA_DIR = Path("../data/embedding_data")
MODEL_FILES = [
    {
        'name': 'config.json',
        'url': 'config.json',
...
def setup_embedding():
    """Download all-MiniLM-L6-v2 model files."""
    try:
        # Create embedding data directory if it doesn't exist
        EMBEDDING_DATA_DIR.mkdir(parents=True, exist_ok=True)
        logger.info(f"Created embedding data directory at {EMBEDDING_DATA_DIR}")

        # Base URL for all-MiniLM-L6-v2 model files
        base_url = "https://huggingface.co/sentence-transformers/all-MiniLM-L6-v2/resolve/main"

        # Download each model file
        for file_info in MODEL_FILES:
            file_name = file_info['name']
            file_path = EMBEDDING_DATA_DIR / file_name
        ...
        missing_files = [
            ...
            if not (EMBEDDING_DATA_DIR / f['name']).exists()
        ]
        if missing_files:
            logger.error(f"Missing files: {missing_files}")
            return False

        logger.info("all-MiniLM-L6-v2 model files downloaded successfully!")
        return True
