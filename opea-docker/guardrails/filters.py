...
# Try to load spacy model for more advanced NLP capabilities
try:
    nlp = spacy.load("en_core_web_sm")
except OSError:
    # If model not found, download a small version
    import subprocess
...
class ContentFilter:
    ...
    def contains_profanity(self, text: str) -> Tuple[bool, str]:
        ...
        if pf.is_profane(text):
            ...
            return True, f"Content contains profanity"
        return False, ""
    
    def analyze_intent(self, text: str) -> Tuple[bool, str]:
        """Analyze the intent of the text using NLP."""
        doc = nlp(text)
        
        # Check for potentially harmful intents
        harmful_verbs = ["kill", "hurt", "harm", "attack", "destroy", "damage"]
        for token in doc:
            if token.pos_ == "VERB" and token.lemma_.lower() in harmful_verbs:
                return True, f"Content contains potentially harmful intent: {token.text}"
